...
from util.choice import ChoiceField, CustomChoicesMeta
...
def application_directory_path(instance, filename):
    ...
    if instance.universal_app.org is not None:
        return "orgs/{0}/apps/{1}/{2}/icons/{3}".format(
            instance.universal_app.org.id, instance.universal_app.id, os, name
        )
    else:
        return "users/{0}/apps/{1}/{2}/icons/{3}".format(
            instance.universal_app.owner.id, instance.universal_app.id, os, name
        )
...
def universal_app_directory_path(instance, filename):
    ...
    if instance.org is not None:
        ...
        return "users/{0}/apps/{1}/icons/{2}".format(
            instance.owner.id, instance.id, name
        )
...
class Application(models.Model):
    ...
    description = models.CharField(
        max_length=1024, help_text="A short text describing the app"
    )
    icon_file = models.ImageField(upload_to=application_directory_path)
    os = models.IntegerField(
        choices=OperatingSystem.choices, help_text="The OS the app will be running on"
    )
    universal_app = models.ForeignKey(
        "application.UniversalApp", on_delete=models.CASCADE
    )
    build_type = models.CharField(max_length=128, default="Debug,Release")
    create_time = models.DateTimeField(auto_now_add=True)
    update_time = models.DateTimeField(auto_now=True)


class UniversalApp(models.Model):
    iOS = models.OneToOneField(
        Application,
        blank=True,
        null=True,
        related_name="iOS",
    ...
    android = models.OneToOneField(
        Application,
        blank=True,
        null=True,
        related_name="Android",
        on_delete=models.SET_NULL,
    )
    macOS = models.OneToOneField(
        Application,
        blank=True,
        null=True,
        related_name="macOS",
    ...
    path = models.SlugField(
        max_length=32, help_text="The path of the universal application."
    )
    name = models.CharField(max_length=128, help_text="The descriptive name of the app")
    description = models.CharField(
        max_length=1024, help_text="A short text describing the app"
    ...
    def enable_os_enum_list(self):
        ...
        try:
            if obj.android:
                ret.append(
                    choices.to_representation(Application.OperatingSystem.Android)
                )
        except:  # noqa: E722
            pass
        try:
        ...
        try:
            if obj.windows:
                ret.append(
                    choices.to_representation(Application.OperatingSystem.Windows)
                )
        except:  # noqa: E722
            pass
        try:
