...
from application.models import (AppAPIToken, Application, UniversalApp,
                                UniversalAppUser, Webhook)
...
class UniversalAppSerializer(serializers.ModelSerializer):
    ...
    name = serializers.CharField(
        max_length=128, help_text="The descriptive name of the app"
    )
    description = serializers.CharField(
        required=False,
        max_length=1024,
        allow_blank=True,
        help_text="A short text describing the application",
    )
...
class UniversalAppCreateSerializer(serializers.Serializer):
    ...
    visibility = ChoiceField(VisibilityType.choices)
    enable_os = serializers.ListField(
        child=ChoiceField(choices=Application.OperatingSystem.choices),
        allow_empty=False,
    )
